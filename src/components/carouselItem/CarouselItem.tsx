  ...
  <CSSTransition 
    ...
    className="item">
      {props.navActionHandlers.length > 0 ? 
        <div
        onMouseEnter={(event:any) => props.navActionHandlers.forEach((handler:INavItemActionHandler) => handler.handleItemHoverEnter(event, props.yNavOffset, props.xNavOffset))}
        onMouseLeave={(event:any) => props.navActionHandlers.forEach((handler:INavItemActionHandler) => handler.handleItemHoverExit(event, props.yNavOffset, props.xNavOffset))}
        onMouseMove={(event:any) => props.navActionHandlers.forEach((handler:INavItemActionHandler) => handler.handleItemHover(event, props.yNavOffset, props.xNavOffset))}
        onClick={(event:any) => props.navActionHandlers.forEach((handler:INavItemActionHandler) => handler.handleItemOnClick(event, props.yNavOffset, props.xNavOffset))}
        className="item">
